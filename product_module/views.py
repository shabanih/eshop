...
class ProductListView(ListView):
    ...
    def get_queryset(self):
        ...
        if brand_name is not None:
            query = query.filter(brand__title_urls__exact=brand_name)

        if category_name is not None:
            query = query.filter(category__title_urls__exact=category_name)
        return query
...
class ProductDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # favorite_product_id = request.session.get('products_favorite')
...
def product_component(request: HttpRequest):
    ...
    return render(request, 'components/product_component.html', context)
...
def product_brands(request: HttpRequest):
    ...
    return render(request, 'components/product_component_cunt.html', context)